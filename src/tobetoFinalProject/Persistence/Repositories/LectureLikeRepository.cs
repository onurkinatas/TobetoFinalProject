...
public class LectureLikeRepository : EfRepositoryBase<LectureLike, Guid, BaseDbContext>, ILectureLikeRepository
{
    ...
    public Task<int> GetLectureLikeCount(Expression<Func<LectureLike, bool>> filter) => Context.Set<LectureLike>().Where(e => e.DeletedDate == null).Where(filter).ToList().Count;
}